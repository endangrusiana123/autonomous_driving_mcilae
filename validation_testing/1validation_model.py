...
def game_loop(args):
    ...
    try:
        ...
        weather_name = ""
        if (args.vw):
            world.world.set_weather(carla.WeatherParameters.SoftRainSunset)
        ...
        if args.model_arch in ('mul_cil', 'mul_cil_f', 'mul_cil_mb', 'mul_cil_minirs', 'mul_cil_a2'):
            model_type = 1
        else: 
        ...
        # Cetak header CSV
        # print("Max acceleration,Min acceleration,Max deceleration,Min deceleration,Dominant speed,Count,Avg speed, Episode success")

        # Cetak data CSV (semua dalam 1 baris dipisahkan koma)
        # print(f"{fmt(max_accel_mps2)},{fmt(min_accel_mps2)},{fmt(max_decel_mps2)},{fmt(min_decel_mps2)},"
        #     f"{dominant_speed_bin if dominant_speed_bin is not None else 'N/A'},{count},{avg_speed},{episode_success}")

        # Nama file CSV
        name_vwt = ""
        if args.vw and not args.vt:
            name_vwt = "vw"
        elif not args.vw and args.vt:
            name_vwt = "vt"
        elif args.vw and args.vt:
            name_vwt = "vwt"
        
        csv_file = f"/opt/carla-simulator/PythonAPI/examples/validation/{args.model_arch}_{args.run}_{args.model_saved}_{name_vwt}_{args.task}.csv"
...
def main():
    ...
    argparser.add_argument(
        "--model_arch",
        type=str,
        choices=["cil", "mul_cil", "mul_cil_f", "mul_cil_mb", 'mul_cil_minirs', 'mul_cil_a2'],
        required=True,
